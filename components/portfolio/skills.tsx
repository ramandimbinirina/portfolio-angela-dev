  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <div 
          className="p-3 bg-primary/10 rounded-xl transition-all duration-300 transform group-hover:rotate-6 flex items-center justify-center text-primary"
        >
          <div className="w-6 h-6">{skill.icon}</div>
        </div>
        <div>
          <h4 className="text-sm font-bold text-foreground dark:text-white tracking-tight">{skill.name}</h4>
          {skill.level && (
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div ref={ref} className={`mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <h2 className="text-4xl sm:text-6xl font-bold text-foreground dark:text-white mb-6 tracking-tight">
            Compétences <span className="text-foreground/40 dark:text-white/40 font-light">Techniques</span>
          </h2>
          <p className="text-lg text-foreground/50 dark:text-white/60 max-w-2xl leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
          {skillCategories.map((category, catIndex) => (
            <div key={category.name} className="space-y-6">
              <h3 className="text-xs font-black uppercase tracking-[0.4em] text-foreground/30 dark:text-white/40 pl-2">
                {category.name}
              </h3>
